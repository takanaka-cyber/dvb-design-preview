...
（`ui_kits/dvb/AdminScreens.jsx` `AdminScreens2.jsx`。新規部品なし。既存部品への追加 3 点を先に入れる）
- 既存部品の追加（props・見た目は変えず追加のみ）: `Badge` に kind `status` の値追加（承認待ち / 研修中 / 承認済 / 却下、送信済 / 未記載 / 休、未納品 / 納品済 / タスクなし。日本語ラベルでも引ける）と kind `tier`（Tier 1 = primary-subtle / Tier 2 = info-subtle / Tier 3 = muted）・`role`（ADMIN = primary-subtle / MANAGER・LEADER = info-subtle / 他 = muted、mono 11px）。`DataTable` に `editingKey` と列の `edit(row)`（行内インライン編集: 行が Input に変わり、左 2px `--primary` + `--primary-subtle` 背景、行クリック無効）。`PageHeader` に `backLink`（h1 の上に「← 戻る」13px muted）。バッチ 1〜3 でキット内に仮置きした 3 点はこの本体実装に寄せる。
- 管理画面の共通: h1 左・右側は基本空・帯色なし・Card 縦積み。幅は `max-w-4xl`（896）に統一（E3。システム設定だけ現状の `max-w-2xl` 672 を維持、研修管理は SubNav 200 + 本文 896）。h1 は PageHeader 標準（`text-xl` の 2 画面を揃える）。confirm 約 20・alert 約 45 → `ConfirmDialog` / `Toast`。
- **型 A（インライン CRUD リスト）** = `CrudList`: Card「新規◯◯登録」（Input + 右端「登録」primary）→ Card「登録済み◯◯（N 件）」（`DataTable editingKey`、行右端 ✏️ ghost / 🗑 ghost destructive → 編集中は ✓ primary / × ghost、Enter 確定・Esc 取消、🗑 は `ConfirmDialog` + 取り消し Toast）。見本 = 案件マスタ `/admin/projects`（通常・空・編集中・削除確認）。該当: 案件別まとめ管理（✏️🗑 を ghost に）/ Meta BM / TikTok BC / 組織管理・リーダー管理（ユーザー管理に埋め込み）。`embedded` で 1 Card（見出し → 登録行 → 表）に畳む。
- **型 B（設定フォーム）** = `ChatworkSettingsForm`: Card × N（`SectionHeading` = 設定グループ、`Field` 縦積み、行追加は Label 右の secondary、行削除は行内右の ghost destructive）→ 最下部「保存」primary 1 個（左寄せ）。保存中 = disabled + spinner、結果は Toast。エラー = 帯 `--negative-subtle` + `Field error`。該当: 目標設定（四半期目標）/ システム設定のチャットワーク連携。CP判断設定は描かない（休眠）。
- **型 C（Dialog + 表）** = `FetchJobsPanel`: 右端「追加」primary → `Dialog sm`（フッタ キャンセル / 追加）→ 進捗パネル（`Progress` + 開始 / 停止 / 再試行 / リセット、`--muted` 帯）→ `DataTable`（行末 🗑 ghost destructive → ConfirmDialog）。見本 = 追加取得 Meta（通常・空・Dialog 開）。該当: 追加取得 TikTok（同じ見た目）/ 共有CP管理（週次まとめ管理に埋め込み）/ ユーザー追加。
- **型 D（読み取り専用ダッシュボード）**: PageHeader（右に 週 Select + 「更新」secondary、または空）→ `KpiCard` × 4 → Card（アコーディオン行 or DataTable）。読み込み = KpiCard loading + Skeleton 行、エラー = 帯 + KpiCard error、空 = `EmptyState`。見本 = 週次まとめ管理 `/admin/weekly-reports`（帯🤝 → `SectionHeading icon="Handshake"`、要因ブロックの blue / red → `--positive-subtle` / `--negative-subtle`、行右 外部リンク）。該当: 利用状況（DAU 棒は `--primary`、土日は `--muted`。ナビには載せない）/ 研修管理。
- ユーザー管理 `/admin/users`（A + C）: h1 右「ユーザー追加」→ Dialog。承認待ち（黄 → 白 + `Badge status="pending"`、行右 承認 primary / 却下 ghost negative）→ 研修中（青 → `training`）→ 承認済み（部署 `Tabs` + DataTable、ROLE 色 6 種 → `Badge kind="role"`、行右 `⋯` メニュー: 権限 / 所属 / 研修中 / パスワードリセット / 無効化 destructive → ConfirmDialog）→ 却下済み（薄）→ 組織管理（slate 帯・indigo→purple 廃止、部 → 課の入れ子はそのまま）→ リーダー管理（課ごとに kit Select）。単独ルート `/admin/organization` `/admin/leaders` は描かない。
- アセクリ・ルーム管理 `/admin/assignees`（A × 2）: 2 カラム維持。追加ボタンは**リストの下・全幅 secondary**（位置同じ）→ 破線パネルのフォーム（キャンセル / 追加）。Tier は `Badge kind="tier"`。下部に Tier 集計。confirm 3・alert 8 → ConfirmDialog / Toast。
- 目標設定 `/admin/evaluation-settings`（B + D）: 四半期ピル → `SegmentedControl`（PageHeader 右）、自前 `<a>` タブ → `Tabs`（四半期目標 / 進捗一覧 / 評価者割当）。四半期目標は型 B（Textarea × 2 + 期限 → 保存 左寄せ）。進捗一覧は KpiCard 4 + DataTable（名前 / 所属 / 等級 / ステータス Badge / 更新日）。評価者割当は DataTable + kit Select（役職のみ、個人名なし）。
- システム設定 `/admin/settings`: `max-w-2xl`、Card 8 枚の順序維持: 登録候補（右上「検出」secondary）/ 追加依頼（黄 → 白 + `--warning` Badge）/ Meta BM（型 A embedded）/ 追加取得 Meta（型 C embedded）/ TikTok BC / 追加取得 TikTok（Meta と同じ見た目）/ チャットワーク連携（型 B embedded、下部 保存）/ ヘルプ。
- 研修管理 `/admin/onboarding`（D）: h1 標準（E3）。研修生 `Select`（PageHeader 右）+ §3.3.1 と同じ `SubNav`（`navGroups` を共用、閲覧モード）。ホーム = KpiCard 4 + 今の Step + 送った FB。学習リスト / バリュー / CR は読み取り専用の表・一覧 + 下部に FB 記入欄（Textarea + 送信 primary）だけ操作可。表示名「研修生の進捗」（担当者確認）。セグメント 6（chat / checklist / lectures / books / values / cr-output）は SubNav の項目に対応させる。
- 描かない: CP判断設定（休眠）/ リーダー管理・組織管理（ユーザー管理内）/ 共有CP（週次まとめ管理内）。
- ナビ: `NAV_TO_SCREEN` に admin-* 8 項目を追記（`Sidebar` のグループ定義は変えない）。利用状況の導線は未確認（§未確認 1）。
