データテーブル。thead は --muted、数値は右寄せ tabular、並べ替え矢印は sortable 列だけ。行 44px（compact 36px）。loading / error / 空の3状態を内包。`expandedKeys` + `renderExpanded(row)` で行の直下に全列幅の展開領域（備考・検証CP要約）を出せる。
